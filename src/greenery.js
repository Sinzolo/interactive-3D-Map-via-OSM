'use strict';
const naturalFeaturesFetchWorker = new Worker('/src/fetchWorker.js');
    ...
    return new Promise((resolve, reject) => {
        ...
        heightMaps.then(({ windowedTwoDHeightMapArray, twoDHeightMapArray }) => {
            Promise.all([windowedTwoDHeightMapArray, twoDHeightMapArray]).then(([_unused, heightMap]) => {
                try {
                    newGrassArea.object3D.position.set((pixelCoords.x * areaCoordsScale), (heightMap[pixelCoords.roundedX][pixelCoords.roundedY]), (pixelCoords.y * areaCoordsScale));
                } catch {
